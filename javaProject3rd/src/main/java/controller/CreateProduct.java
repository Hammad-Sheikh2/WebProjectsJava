...
public class CreateProduct  extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            DatabaseHandler dbhandler = new DatabaseHandler();
            int r = dbhandler.insertQuery("INSERT INTO Product(name, scale, vendor, pdtdescription, qtyinstock, buyprice, msrp, code, category, image, review, information) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
                    name, scale, vendor, PdtDescription, qtyInStock, buyPrice, msrp, code, category, file.getSubmittedFileName(), descText, descHtml);
            if(r>0)
            {
                request.setAttribute("error", "Product created");
                response.sendRedirect("./product?id="+r);
                return;
            }
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
